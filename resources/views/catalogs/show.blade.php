            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex space-x-4">
                    ...
                    <form action="{{ route('catalogs.destroy', $catalog->id) }}" method="POST">
                        ...
                        @method('DELETE')
                        <button type="submit" 
                                onclick="return confirm('Tem certeza que deseja excluir esse procedimento?')"
            ...
            <div class="p-10 rounded-2xl shadow-lg" style="border: 2px solid #58A663; background-color: #FFECEF;">
                ...
                <div class="grid grid-cols-2 gap-x-12 gap-y-6">
                    ...
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        ...
                        <p class="text-xl font-medium text-gray-800">{{$catalog->name}}</p>
                    </div>

                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Valor:</span>
                        <p class="text-xl font-medium text-gray-800">R$ {{ number_format($catalog->price, 2, ',', '.') }}</p>
                    </div>
                    
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Tempo médio:</span>
                        <p class="text-xl font-medium text-gray-800">{{$catalog->average_time}} minutos</p>
                    </div>
                    
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Materiais usados:</span>
                        <p class="text-xl font-medium text-gray-800">{{$catalog->materials_used}}</p>
                    </div>

                    <div class="col-span-2 bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Descrição:</span>
                        <p class="text-xl font-medium text-gray-800">{{$catalog->description}}</p>
                    </div>

                    <div class="col-span-2 bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Contraindicações:</span>
                        <p class="text-xl font-medium text-gray-800">{{$catalog->contraindications}}</p>
                    </div>
