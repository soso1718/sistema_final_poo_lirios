<x-app-layout>
    <div class="min-h-screen bg-gray-100 pb-10">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
            ...
            <form action="{{ route('schedulings.update', $scheduling->id) }}" method="POST" id="scheduling-edit-form">
                @method('PUT')
                @csrf
                ...
                @if ($errors->any())
                    <div class="p-4 mb-4 rounded-lg bg-red-100 text-red-700 font-medium">
                        <p>Houve um erro ao tentar editar o agendamento:</p>
                ...
                <div class="p-10 rounded-2xl shadow-lg" style="border: 2px solid #58A663; background-color: #FFECEF;">
                    ...
                    <div class="grid grid-cols-2 gap-x-12 gap-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="patient_name" required value="{{ old('patient_name', $scheduling->patient_name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="patient_id" required value="{{ old('patient_id', $scheduling->patient_id) }}"
                                class="w-full py-3 px-4 rounded-xl border-none shadow-inner text-lg transition duration-150"
                                style="background-color: #FDE7EE; color: #333333;">
                        </div>
                            ...
                            <select name="service" id="service" required
                                ...
                                @foreach ($catalog as $item)
                                    <option value="{{ $item->name }}" 
                                        {{ old('service', $scheduling->service) === $item->name ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="professional" id="professional" required
                                ...
                                @foreach ($professionals as $p)
                                    <option value="{{ $p->name }}" 
                                        {{ old('professional', $scheduling->professional) === $p->name ? 'selected' : '' }}>
                                        {{ $p->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="date" id="date" required value="{{ old('date', $scheduling->date) }}"
                                class="w-full py-3 px-4 rounded-xl border-none shadow-inner text-lg transition duration-150"
                                style="background-color: #FDE7EE; color: #333333;">
                        </div>
                            ...
                            <select name="time" id="time" required
                                ...
                                @php
                                    ...
                                    while ($start <= $end) {
                                        ...
                                        $selected = old('time', $scheduling->time) == $timeFormatted ? 'selected' : '';
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="text" name="notes" value="{{ old('notes', $scheduling->notes) }}"
                                class="w-full py-3 px-4 rounded-xl border-none shadow-inner text-lg transition duration-150"
                                style="background-color: #FDE7EE; color: #333333;">
                        </div>
