            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex space-x-4">
                    ...
                    <form action="{{ route('schedulings.destroy', $scheduling->id) }}" method="POST">
                        ...
                        @method('DELETE')
                        <button type="submit" 
                                onclick="return confirm('Tem certeza que deseja excluir esse agendamento?')"
            ...
            <div class="p-10 rounded-2xl shadow-lg" style="border: 2px solid #58A663; background-color: #FFECEF;">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6 mb-6">
                    ...
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Paciente:</span>
                        <p class="text-xl font-medium text-gray-800">{{$scheduling->patient_name}}</p>
                    </div>

                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">CPF:</span>
                        <p class="text-xl font-medium text-gray-800">{{$scheduling->patient_id}}</p>
                    </div>
                    
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Serviço:</span>
                        <p class="text-xl font-medium text-gray-800">{{$scheduling->service}}</p>
                    </div>
                    
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Data:</span>
                        <p class="text-xl font-medium text-gray-800">{{$scheduling->date}}</p>
                    </div>
                    
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Horário:</span>
                        <p class="text-xl font-medium text-gray-800">{{$scheduling->time}}</p>
                    </div>
                    
                    <div class="bg-white p-4 rounded-xl shadow-md" style="background-color: #FAFAFA;">
                        <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Profissional:</span>
                        <p class="text-xl font-medium text-gray-800">{{$scheduling->professional}}</p>
                    </div>
                </div>

                <div class="bg-white p-4 rounded-xl shadow-md mt-6" style="background-color: #FAFAFA;">
                    <span class="block text-lg font-serif mb-1" style="color: #F0A5B7;">Observações:</span>
                    <p class="text-xl font-medium text-gray-800">{{$scheduling->notes ?? 'Nenhuma observação cadastrada.'}}</p>
                </div>
